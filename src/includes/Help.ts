...
export default {
  ...
  execute: async function (message: Message, others: any) {
    const { commands, prefix, commandCategories } = others;

    // @ts-ignore
    const client: ModifiedClient = message.client;
    ...
    if (!args[1]) {
      ...
      const embed = client.djsCommandControl
        .getEmbed()
        .setTitle("Help Categories")
        .setDescription(
          `Simply type \`${prefix}help {{category name}}\` to see the commands in that category.\nCategory names are in codeblocks below each category`
        );

      // Add each category to the embed
      Object.keys(categories).forEach((key) => {
        const category = commandCategories.find(
          (item: Category) => item.name === key
        );
        if (category) {
      ...
      if (Object.keys(categories).includes(category)) {
        ...
        const categoryEnum = commandCategories.find(
        ...
        const embed = client.djsCommandControl
          ...
          .setDescription(
            `Commands in category: \`${category.toLowerCase()}\``
          );
        ...
        categories[category].forEach((command: Command) => {
          ...
          embed.addField(`${command.name}`, description, false);
        });
        ...
        return message.channel.send(
          client.djsCommandControl
            ...
            .setTitle("Help Error")
            .setDescription(
              "Sorry, but either that category does not exist, or you do not have permission to user any of the command in that category"
            )
        );
      }
    }
